...
make_build_coverage(){
  ...
  $HOME/lcov/bin/lcov --gcov-tool $gcov_path --remove $total '/Applications/Xcode.app/*' '*/Pods/*' '*/Library/Helper/*' -o $filter --rc lcov_branch_coverage=1 > $log_file
  if [[ ! -f "$filter" ]]; then
    #statements
    cat $log_file
    exit 1
